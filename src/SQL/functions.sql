...
DROP FUNCTION IF EXISTS SearchByPublisher(val varchar(10));
...
CREATE FUNCTION SearchByPublisher(val varchar(400))
	RETURNS TABLE(
		isbn varchar(10),
		quantity integer,
		book_name varchar(255),
		...
		RETURN QUERY
			...
			((public.book inner join public.author on public.author.isbn = public.book.isbn) inner join public.publisher on public.publisher.publisher_id = public.book.publisher_id) left join public.genre on public.genre.isbn = public.book.isbn
			where
			public.publisher.publisher_id = SearchByPublisher.val OR
			public.publisher.publisher_id ~ SearchByPublisher.val
			group by book.isbn,public.publisher.publisher_id;
		END;
$$ LANGUAGE plpgsql;
...
CREATE FUNCTION SearchByAtomic(val varchar(400), col_nam varchar(400))
	RETURNS TABLE(
		...
		RETURN QUERY EXECUTE format('
			select book.isbn,book.quantity,book.book_name,book.number_of_pages,book.unit_price,book.date_of_publish,CONCAT((public.publisher.name).first_name,'' '',(public.publisher.name).last_name) as "publisher_name", string_agg(public.author.author_name, '', '') as authors,string_agg(DISTINCT public.genre.genre, '', '') as genres
			from
...
select * from SearchByAtomic('10','book.isbn');
